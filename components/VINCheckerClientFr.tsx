...
const VIN_LOCATIONS = [
  { n: '1', spot: 'Plaque du tableau de bord', detail: 'Visible à travers le pare-brise, côté conducteur.' },
  { n: '2', spot: "Étiquette de la portière", detail: "Dans l'encadrement de la portière conducteur — doit correspondre au tableau de bord." },
  { n: '3', spot: 'Gravé sur le châssis', detail: 'Gravé sur le longeron du châssis, sous le capot.' },
  { n: '4', spot: 'Coffre / sous le siège', detail: "Sur certains modèles, il est gravé sous le tapis du coffre ou du siège avant." },
];
...
export default function VINCheckerClientFr() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('Merci de saisir un numéro VIN.'); return; }
    if (!isValidVIN(cleaned)) { setError('Le VIN doit contenir exactement 17 caractères — uniquement des lettres (sans I, O, Q) et des chiffres.'); return; }
    ...
    } catch { setError('Impossible de se connecter à la base de données VIN. Vérifiez votre connexion et réessayez.'); }
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="max-w-2xl mb-8">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                placeholder="Saisissez le VIN / numéro de châssis (17 caractères)"
            ...
            <button onClick={decode} disabled={loading || vin.length < 17}
              ...
              {loading ? <><div className="w-4 h-4 border-2 border-muted-foreground/30 border-t-foreground rounded-full animate-spin" />Décodage...</> : <><Search className="h-4 w-4" />Décoder</>}
            </button>
          </div>
          ...
          <div className="flex justify-between items-center mt-1">
            <span className="text-xs text-muted-foreground">{vin.length}/17 caractères</span>
            {vin.length === 17 && <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">Prêt à décoder</span>}
          </div>
          {error && (
          ...
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">Données limitées — véhicule hors Amérique du Nord</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">Ce VIN n'est pas dans la base de données NHTSA (origine européenne, asiatique ou autre). Année-modèle décodée à partir de la structure du VIN{result.modelYear ? ` : ${result.modelYear}` : ''}. Pour des spécifications complètes, contactez le constructeur.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">VIN décodé — véhicule nord-américain confirmé</p>
                <button onClick={reset} className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> Nouvelle recherche</button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(result.engineCylinders || result.engineDisplacementL || result.engineHP) && (
                <SpecCard icon={<Zap className="h-3.5 w-3.5 text-orange-500" />} title="Moteur">
                  {result.engineCylinders && <Row label="Cylindres" value={result.engineCylinders} />}
                  {result.engineDisplacementL && <Row label="Cylindrée" value={result.engineDisplacementL + 'L'} />}
                  {result.engineHP && <Row label="Puissance" value={result.engineHP + ' ch'} />}
                </SpecCard>
              )}
              {(result.manufacturer || result.plantCountry) && (
                <SpecCard icon={<Globe className="h-3.5 w-3.5 text-blue-500" />} title="Origine">
                  {result.manufacturer && <Row label="Constructeur" value={result.manufacturer} />}
                  {result.plantCountry && <Row label="Pays" value={`${getOriginFlag(result.plantCountry)} ${result.plantCountry}`} />}
                  {result.plantCity && <Row label="Usine" value={result.plantCity} />}
                </SpecCard>
              )}
              {(result.modelYear || result.doors || result.transmissionStyle) && (
                <SpecCard icon={<Car className="h-3.5 w-3.5 text-emerald-500" />} title="Véhicule">
                  {result.modelYear && <Row label="Année" value={result.modelYear} />}
                  {result.doors && <Row label="Portes" value={result.doors} />}
                  {result.transmissionStyle && <Row label="Transmission" value={result.transmissionStyle} />}
                </SpecCard>
              )}
              {(result.abs || result.airBagLocFront) && (
                <SpecCard icon={<Shield className="h-3.5 w-3.5 text-emerald-500" />} title="Sécurité">
                  {result.abs && <Row label="ABS" value={result.abs} />}
                  {result.airBagLocFront && <Row label="Airbags" value={result.airBagLocFront} />}
                  {result.seatBeltsAll && <Row label="Ceintures" value={result.seatBeltsAll} />}
                </SpecCard>
              )}
            </div>

            {/* Rappel de vérification physique du VIN */}
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">Vérifiez le VIN aux trois emplacements physiques</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href="/outils/mecanicien-virtuel" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group">
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">Diagnostiquer cette voiture</p>
                <ChevronRight className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-500" />
              </Link>
              <Link href="/outils/combien-vaut-ma-voiture" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group">
                <p className="text-xs font-bold text-blue-700 dark:text-blue-400">Estimer sa valeur</p>
                <ChevronRight className="h-3.5 w-3.5 text-blue-600 dark:text-blue-500" />
              </Link>
            </div>
          </div>
        ) : (
          /* Guide avant recherche */
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Où Trouver le VIN</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Structure du VIN</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'Pays et constructeur' },
                  { pos: '4–8', name: 'VDS', desc: 'Modèle, moteur, carrosserie' },
                  { pos: '9', name: 'Chiffre de contrôle', desc: 'Valide le VIN complet' },
                  { pos: '10', name: 'Année-modèle', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: 'Usine', desc: "Code de l'usine d'assemblage" },
                  { pos: '12–17', name: 'Série', desc: 'Numéro de production unique' },
                ].map(({ pos, name, desc }) => (
