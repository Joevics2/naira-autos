...
const VIN_LOCATIONS = [
  { n: '1', spot: 'ダッシュボード', detail: 'フロントガラス越し、運転席側から見えるプレート。' },
  { n: '2', spot: 'ドア枠のステッカー', detail: '運転席ドアの枠内 — ダッシュボードの番号と一致するはず。' },
  { n: '3', spot: 'フレームへの刻印', detail: 'エンジンルーム内、フレームに直接刻印。' },
  { n: '4', spot: 'トランク／シート下', detail: '車種によってはトランクやシート下のカーペットの下に刻印。' },
];
...
export default function VINCheckerClientJa() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('VINコード（車台番号）を入力してください。'); return; }
    if (!isValidVIN(cleaned)) { setError('VINコードは17桁で入力してください — アルファベット（I、O、Qを除く）と数字のみです。'); return; }
    ...
    } catch { setError('VINデータベースに接続できませんでした。インターネット接続を確認して、もう一度お試しください。'); }
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="max-w-2xl mb-8">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                placeholder="VINコード／車台番号を入力（17桁）"
            ...
            <button onClick={decode} disabled={loading || vin.length < 17}
              ...
              {loading ? <><div className="w-4 h-4 border-2 border-muted-foreground/30 border-t-foreground rounded-full animate-spin" />解析中...</> : <><Search className="h-4 w-4" />解析する</>}
            </button>
          </div>
          ...
          <div className="flex justify-between items-center mt-1">
            <span className="text-xs text-muted-foreground">{vin.length}/17桁</span>
            {vin.length === 17 && <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">解析の準備ができました</span>}
          </div>
          {error && (
        ...
        {result ? (
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">データが限られています — 北米以外で生産された車両</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">このVINコードはNHTSAデータベースに登録されていません（欧州・アジアなど他地域での生産）。年式はVINコードの構造から算出しています{result.modelYear ? `：${result.modelYear}` : ''}。詳細な仕様はメーカーへお問い合わせください。</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">VINコードを解析しました — 北米生産車両を確認</p>
                <button onClick={reset} className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> 新しく調べる</button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(result.engineCylinders || result.engineDisplacementL || result.engineHP) && (
                <SpecCard icon={<Zap className="h-3.5 w-3.5 text-orange-500" />} title="エンジン">
                  {result.engineCylinders && <Row label="気筒数" value={result.engineCylinders} />}
                  {result.engineDisplacementL && <Row label="排気量" value={result.engineDisplacementL + 'L'} />}
                  {result.engineHP && <Row label="出力" value={result.engineHP + ' hp'} />}
                </SpecCard>
              )}
              {(result.manufacturer || result.plantCountry) && (
                <SpecCard icon={<Globe className="h-3.5 w-3.5 text-blue-500" />} title="生産地">
                  {result.manufacturer && <Row label="メーカー" value={result.manufacturer} />}
                  {result.plantCountry && <Row label="国" value={`${getOriginFlag(result.plantCountry)} ${result.plantCountry}`} />}
                  {result.plantCity && <Row label="工場" value={result.plantCity} />}
                </SpecCard>
              )}
              {(result.modelYear || result.doors || result.transmissionStyle) && (
                <SpecCard icon={<Car className="h-3.5 w-3.5 text-emerald-500" />} title="車両情報">
                  {result.modelYear && <Row label="年式" value={result.modelYear} />}
                  {result.doors && <Row label="ドア数" value={result.doors} />}
                  {result.transmissionStyle && <Row label="トランスミッション" value={result.transmissionStyle} />}
                </SpecCard>
              )}
              {(result.abs || result.airBagLocFront) && (
                <SpecCard icon={<Shield className="h-3.5 w-3.5 text-emerald-500" />} title="安全装備">
                  {result.abs && <Row label="ABS" value={result.abs} />}
                  {result.airBagLocFront && <Row label="エアバッグ" value={result.airBagLocFront} />}
                  {result.seatBeltsAll && <Row label="シートベルト" value={result.seatBeltsAll} />}
                </SpecCard>
              )}
            </div>
            ...
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">VINコードは車両の3箇所で現物確認を</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href="/tsuru/ai-shindan" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group">
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">この車を診断する</p>
                <ChevronRight className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-500" />
              </Link>
              <Link href="/tsuru/kuruma-satei" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group">
                <p className="text-xs font-bold text-blue-700 dark:text-blue-400">この車を査定する</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">VINコードの確認場所</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">VINコードの構造</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: '生産国とメーカー' },
                  { pos: '4–8', name: 'VDS', desc: 'モデル・エンジン・車体' },
                  { pos: '9', name: 'チェックデジット', desc: 'VIN全体の正当性を検証' },
                  { pos: '10', name: '年式', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: '組立工場', desc: '組立工場のコード' },
                  { pos: '12–17', name: '製造番号', desc: '個体を識別する固有番号' },
                ].map(({ pos, name, desc }) => (
