 ...
 * verified-distance pairs, popular routes, and currency — nothing about
 * distance/time/fuel logic or UI should ever need to change per country.
 */
...
function TownSelect({
  label, value, onChange, exclude, towns,
}: {
  label: string;
  value: string;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-2 items-end">
        <TownSelect label="From" value={from} onChange={setFrom} exclude={to} towns={towns} />
        <button
          onClick={swap}
        ...
        <TownSelect label="To" value={to} onChange={setTo} exclude={from} towns={towns} />
      </div>
        ...
        <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10 space-y-3">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-sm font-black text-amber-400">{fmtMoney(pumpPrice, currencySymbol)}</span>
            </div>
            <input
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <span className="text-xl font-black text-amber-400" style={{ fontFamily: "'Barlow Condensed', Impact, sans-serif" }}>
              {fmtMoney(fuel.cost, currencySymbol)}
            </span>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 gap-2">
          {relatedTools.map((rt) => (
            ...
            >
              ...
              <ChevronRight className={`h-3.5 w-3.5 ${rt.highlight ? 'text-amber-400' : 'text-white/40'}`} />
            </Link>
          ))}
        </div>
      )}
