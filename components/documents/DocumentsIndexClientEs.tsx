...
interface DocumentsIndexClientEsProps {
...
export default function DocumentsIndexClientEs({ templates }: DocumentsIndexClientEsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Buscar plantillas por nombre o país..."
      ...
      {filtered.length === 0 && (
        <p className="text-sm text-muted-foreground">Ninguna plantilla coincide con &quot;{query}&quot;.</p>
      )}
...
function TemplateCard({ e, showUpdated }: { e: EnrichedTemplate; showUpdated?: boolean }) {
  return (
    <Link
      href={`/plantillas/${e.row.document_type}/${e.row.country}`}
    ...
    >
      ...
      <p className="text-xs text-muted-foreground mt-1">
        ...
        {showUpdated && (
          <>
            {' · '}
            Actualizado {new Date(e.row.updated_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
          </>
        )}
      </p>
    </Link>
