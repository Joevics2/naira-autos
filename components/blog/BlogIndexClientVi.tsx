...
export default function BlogIndexClientVi({ posts }: { posts: BlogPost[] }) {
      ...
      {filteredBlogs.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-muted-foreground text-lg">
            {searchTerm ? 'Không tìm thấy bài viết' : 'Chưa có bài viết tiếng Việt nào — hãy quay lại sau nhé.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBlogs.map((blog) => (
            <Link key={blog.id} href={`/blog-o-to/${blog.slug}`}>
