...
export default function BlogIndexClientTr({ posts }: { posts: BlogPost[] }) {
      ...
      {filteredBlogs.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-muted-foreground text-lg">
            {searchTerm ? 'Yazı bulunamadı' : 'Henüz Türkçe yazı yok — daha sonra tekrar kontrol edin.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBlogs.map((blog) => (
            <Link key={blog.id} href={`/oto-blog/${blog.slug}`}>
