...
export default function BlogIndexClientIt({ posts }: { posts: BlogPost[] }) {
      ...
      {filteredBlogs.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-muted-foreground text-lg">
            {searchTerm ? 'Nessun articolo trovato' : 'Ancora nessun articolo in italiano — torna presto.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBlogs.map((blog) => (
            <Link key={blog.id} href={`/blog-motori/${blog.slug}`}>
              <Card className="h-full hover:shadow-lg transition-all overflow-hidden">
                ...
                <CardContent className="p-4">
                  ...
                  <div className="mt-4 text-xs text-muted-foreground">
                    {new Date(blog.created_at).toLocaleDateString('it-IT', {
                      day: 'numeric',
                      month: 'short',
                      year: 'numeric',
                    })}
