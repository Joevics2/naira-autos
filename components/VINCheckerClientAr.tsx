...
import {
  ...
  Car, Zap, Globe, Shield, RotateCcw, Copy, Check, ChevronLeft
} from 'lucide-react';
...
const VIN_LOCATIONS = [
  { n: '1', spot: 'لوحة تابلوه السيارة', detail: 'تظهر من خلال الزجاج الأمامي، جهة السائق.' },
  { n: '2', spot: 'ملصق إطار الباب', detail: 'داخل إطار باب السائق — يجب أن يطابق اللوحة الموجودة على التابلوه.' },
  { n: '3', spot: 'محفور على الهيكل', detail: 'محفور على دعامة الهيكل، أسفل غطاء المحرك.' },
  { n: '4', spot: 'الشنطة الخلفية / أسفل المقعد', detail: 'بعض الموديلات يكون الرقم محفورًا أسفل سجادة الشنطة أو المقعد الأمامي.' },
];
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-2">
      ...
      <span className="text-xs font-semibold text-foreground text-right" dir="ltr">{value}</span>
    </div>
  );
}
...
function Badge({ color, children }: { color: 'emerald' | 'blue' | 'gray'; children: React.ReactNode }) {
  ...
  return <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${cls}`} dir="ltr">{children}</span>;
}

export default function VINCheckerClientAr() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('من فضلك أدخل رقم الهيكل (VIN).'); return; }
    if (!isValidVIN(cleaned)) { setError('يجب أن يتكون رقم الهيكل من 17 حرفًا بالضبط — حروف إنجليزية (بدون I أو O أو Q) وأرقام فقط.'); return; }
    ...
    } catch { setError('تعذّر الاتصال بقاعدة بيانات VIN. تحقق من اتصالك بالإنترنت وحاول مرة أخرى.'); }
  ...
  return (
    <div dir="rtl" lang="ar" className="bg-background border-t border-border">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">

        {/* شريط إدخال رقم الهيكل */}
        <div className="max-w-2xl mb-8">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin} dir="ltr"
                ...
                placeholder="أدخل رقم الهيكل (VIN) — 17 حرفًا"
                maxLength={17} spellCheck={false}
                className="w-full h-12 px-4 text-sm font-mono tracking-wider text-left border-2 border-muted-foreground rounded-xl bg-background text-foreground placeholder:text-muted-foreground placeholder:font-sans placeholder:tracking-normal focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
              />
              {vin && (
                <button onClick={copyVin} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/50 hover:text-muted-foreground transition-colors">
            ...
            <button onClick={decode} disabled={loading || vin.length < 17}
              ...
              {loading ? <><div className="w-4 h-4 border-2 border-muted-foreground/30 border-t-foreground rounded-full animate-spin" />جارِ الفك...</> : <><Search className="h-4 w-4" />فك الترميز</>}
            </button>
          </div>
          ...
          <div className="flex justify-between items-center mt-1">
            <span className="text-xs text-muted-foreground">{vin.length}/17 حرفًا</span>
            {vin.length === 17 && <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">جاهز لفك الترميز</span>}
          </div>
          {error && (
        ...
        {result ? (
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">بيانات محدودة — سيارة من خارج أمريكا الشمالية</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">رقم الهيكل هذا غير موجود في قاعدة بيانات NHTSA (أصل أوروبي أو آسيوي أو غيره). تم فك سنة الموديل من بنية رقم الهيكل نفسه{result.modelYear ? `: ${result.modelYear}` : ''}. للحصول على مواصفات كاملة، تواصل مع الشركة المصنعة.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">تم فك رقم الهيكل — تأكدت السيارة من أمريكا الشمالية</p>
                <button onClick={reset} className="mr-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> فحص جديد</button>
              </div>
            )}
              ...
              <div className="rounded-2xl border border-border bg-card p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0" dir="ltr">
                    <h2 className="text-2xl font-black text-foreground text-left" style={{ fontFamily: "'Barlow Condensed', Impact, sans-serif" }}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(result.engineCylinders || result.engineDisplacementL || result.engineHP) && (
                <SpecCard icon={<Zap className="h-3.5 w-3.5 text-orange-500" />} title="المحرك">
                  {result.engineCylinders && <Row label="الأسطوانات" value={result.engineCylinders} />}
                  {result.engineDisplacementL && <Row label="سعة المحرك" value={result.engineDisplacementL + 'L'} />}
                  {result.engineHP && <Row label="القوة الحصانية" value={result.engineHP} />}
                </SpecCard>
              )}
              {(result.manufacturer || result.plantCountry) && (
                <SpecCard icon={<Globe className="h-3.5 w-3.5 text-blue-500" />} title="المنشأ">
                  {result.manufacturer && <Row label="الشركة المصنعة" value={result.manufacturer} />}
                  {result.plantCountry && <Row label="الدولة" value={`${getOriginFlag(result.plantCountry)} ${result.plantCountry}`} />}
                  {result.plantCity && <Row label="المصنع" value={result.plantCity} />}
                </SpecCard>
              )}
              {(result.modelYear || result.doors || result.transmissionStyle) && (
                <SpecCard icon={<Car className="h-3.5 w-3.5 text-emerald-500" />} title="السيارة">
                  {result.modelYear && <Row label="سنة الصنع" value={result.modelYear} />}
                  {result.doors && <Row label="الأبواب" value={result.doors} />}
                  {result.transmissionStyle && <Row label="ناقل الحركة" value={result.transmissionStyle} />}
                </SpecCard>
              )}
              {(result.abs || result.airBagLocFront) && (
                <SpecCard icon={<Shield className="h-3.5 w-3.5 text-emerald-500" />} title="السلامة">
                  {result.abs && <Row label="ABS" value={result.abs} />}
                  {result.airBagLocFront && <Row label="الوسائد الهوائية" value={result.airBagLocFront} />}
                  {result.seatBeltsAll && <Row label="أحزمة الأمان" value={result.seatBeltsAll} />}
                </SpecCard>
              )}
            </div>
            ...
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">تحقق من رقم الهيكل في المواقع الثلاثة الفعلية</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href="/adawat/ai-mechanic-arabic" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group">
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">شخّص هذه السيارة</p>
                <ChevronLeft className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-500" />
              </Link>
              <Link href="/kam-qeemat-sayarati" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group">
                <p className="text-xs font-bold text-blue-700 dark:text-blue-400">قيّم هذه السيارة</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">أين تجد رقم الهيكل</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">بنية رقم الهيكل</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'الدولة والشركة المصنعة' },
                  { pos: '4–8', name: 'VDS', desc: 'الموديل، المحرك، الهيكل' },
                  { pos: '9', name: 'رقم التحقق', desc: 'يتحقق من صحة الرقم كاملًا' },
                  { pos: '10', name: 'سنة الموديل', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: 'المصنع', desc: 'رمز مصنع التجميع' },
                  { pos: '12–17', name: 'الرقم التسلسلي', desc: 'رقم الإنتاج الفريد للسيارة' },
                ].map(({ pos, name, desc }) => (
                  <div key={pos} className="flex items-center gap-2.5 p-2.5 rounded-lg bg-card border border-border">
                    <span className="text-xs font-mono font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 px-1.5 py-0.5 rounded flex-shrink-0" dir="ltr">{pos}</span>
                    <div>
