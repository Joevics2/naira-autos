...
const VIN_LOCATIONS = [
  { n: '1', spot: 'Armaturenbrett', detail: 'Sichtbar durch die Windschutzscheibe, auf der Fahrerseite.' },
  { n: '2', spot: 'Türrahmen', detail: 'Im Türrahmen der Fahrertür — muss mit der Nummer am Armaturenbrett übereinstimmen.' },
  { n: '3', spot: 'Eingestanzt im Rahmen', detail: 'Im Motorraum in den Rahmenlängsträger eingestanzt.' },
  { n: '4', spot: 'Kofferraum / unter dem Sitz', detail: 'Bei manchen Modellen unter dem Kofferraum- oder Sitzteppich eingestanzt.' },
];
...
export default function VINCheckerClientDe() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('Bitte gib die Fahrgestellnummer (FIN/VIN) ein.'); return; }
    if (!isValidVIN(cleaned)) { setError('Die Fahrgestellnummer muss genau 17 Zeichen haben — nur Buchstaben (ohne I, O, Q) und Ziffern.'); return; }
    ...
    } catch { setError('Verbindung zur VIN-Datenbank fehlgeschlagen. Bitte Internetverbindung prüfen und erneut versuchen.'); }
  ...
  return (
    <div className="bg-background border-t border-border">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Eingabefeld für die Fahrgestellnummer */}
        <div className="max-w-2xl mb-8">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                placeholder="Fahrgestellnummer / FIN eingeben (17 Zeichen)"
            ...
            <button onClick={decode} disabled={loading || vin.length < 17}
              ...
              {loading ? <><div className="w-4 h-4 border-2 border-muted-foreground/30 border-t-foreground rounded-full animate-spin" />Wird decodiert...</> : <><Search className="h-4 w-4" />Decodieren</>}
            </button>
          </div>
          ...
          <div className="flex justify-between items-center mt-1">
            <span className="text-xs text-muted-foreground">{vin.length}/17 Zeichen</span>
            {vin.length === 17 && <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">Bereit zum Decodieren</span>}
          </div>
          {error && (
          ...
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">Eingeschränkte Daten — Fahrzeug außerhalb Nordamerikas</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">Diese Fahrgestellnummer ist nicht in der NHTSA-Datenbank (europäische, asiatische oder andere Herkunft). Das Baujahr wurde aus dem Aufbau der Nummer selbst ermittelt{result.modelYear ? `: ${result.modelYear}` : ''}. Für vollständige technische Daten wende dich an den Hersteller.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">Fahrgestellnummer decodiert — nordamerikanisches Fahrzeug bestätigt</p>
                <button onClick={reset} className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> Neue Suche</button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(result.engineCylinders || result.engineDisplacementL || result.engineHP) && (
                <SpecCard icon={<Zap className="h-3.5 w-3.5 text-orange-500" />} title="Motor">
                  {result.engineCylinders && <Row label="Zylinder" value={result.engineCylinders} />}
                  {result.engineDisplacementL && <Row label="Hubraum" value={result.engineDisplacementL + 'L'} />}
                  {result.engineHP && <Row label="Leistung" value={result.engineHP + ' PS'} />}
                </SpecCard>
              )}
              {(result.manufacturer || result.plantCountry) && (
                <SpecCard icon={<Globe className="h-3.5 w-3.5 text-blue-500" />} title="Herkunft">
                  {result.manufacturer && <Row label="Hersteller" value={result.manufacturer} />}
                  {result.plantCountry && <Row label="Land" value={`${getOriginFlag(result.plantCountry)} ${result.plantCountry}`} />}
                  {result.plantCity && <Row label="Werk" value={result.plantCity} />}
                </SpecCard>
              )}
              {(result.modelYear || result.doors || result.transmissionStyle) && (
                <SpecCard icon={<Car className="h-3.5 w-3.5 text-emerald-500" />} title="Fahrzeug">
                  {result.modelYear && <Row label="Baujahr" value={result.modelYear} />}
                  {result.doors && <Row label="Türen" value={result.doors} />}
                  {result.transmissionStyle && <Row label="Getriebe" value={result.transmissionStyle} />}
                </SpecCard>
              )}
              {(result.abs || result.airBagLocFront) && (
                <SpecCard icon={<Shield className="h-3.5 w-3.5 text-emerald-500" />} title="Sicherheit">
                  {result.abs && <Row label="ABS" value={result.abs} />}
                  {result.airBagLocFront && <Row label="Airbags" value={result.airBagLocFront} />}
                  {result.seatBeltsAll && <Row label="Gurte" value={result.seatBeltsAll} />}
                </SpecCard>
              )}
            </div>

            {/* Hinweis zur physischen Prüfung */}
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">Fahrgestellnummer an drei Stellen physisch prüfen</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href="/werkzeuge/virtueller-mechaniker" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group">
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">Dieses Auto diagnostizieren</p>
                <ChevronRight className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-500" />
              </Link>
              <Link href="/werkzeuge/was-ist-mein-auto-wert" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group">
                <p className="text-xs font-bold text-blue-700 dark:text-blue-400">Wert dieses Autos ermitteln</p>
                <ChevronRight className="h-3.5 w-3.5 text-blue-600 dark:text-blue-500" />
              </Link>
            </div>
          </div>
        ) : (
          /* Leitfaden vor der Suche */
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Wo die Fahrgestellnummer zu finden ist</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Aufbau der Fahrgestellnummer</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'Land und Hersteller' },
                  { pos: '4–8', name: 'VDS', desc: 'Modell, Motor, Karosserie' },
                  { pos: '9', name: 'Prüfziffer', desc: 'Validiert die gesamte Nummer' },
                  { pos: '10', name: 'Baujahr', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: 'Werk', desc: 'Code des Montagewerks' },
                  { pos: '12–17', name: 'Seriennummer', desc: 'Eindeutige Produktionsnummer' },
                ].map(({ pos, name, desc }) => (
