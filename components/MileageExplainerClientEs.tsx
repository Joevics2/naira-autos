...
import { flagEmoji, COUNTRY_NAMES_ES, CITY_NAME_OVERRIDES_ES } from '@/lib/country-meta';
import type { MileageBenchmark } from '@/lib/mileage-benchmarks';
import MileageShareCard, { hookLine, mainSentence, supportingSentence, type ShareCardData } from '@/components/MileageShareCardEs';
...
function unitLabel(u: Unit) { return u === 'km' ? 'km' : 'millas'; }
function esCityName(name: string): string { return CITY_NAME_OVERRIDES_ES[name] ?? name; }
...
export default function MileageExplainerClient({
  ...
  const allCountries = useMemo(
    () => (enableCountrySelect ? availableCountries().map((c) => ({ code: c.code, name: COUNTRY_NAMES_ES[c.code] ?? c.name })) : []),
    [enableCountrySelect]
  );
  ...
  const shareCardData: ShareCardData | null = useMemo(() => {
    ...
    return {
      ...
      unit,
      fromCity: esCityName(from.name),
      toCity: esCityName(to.name),
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div>
              <label className="block text-xs font-bold text-foreground uppercase tracking-wide mb-1.5">Lectura del odómetro</label>
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-end">
              <div>
                <label className="block text-xs font-bold text-foreground uppercase tracking-wide mb-1.5">Desde</label>
                <select value={fromCity} onChange={(e) => setFromCity(e.target.value)} className={iCls}>
                  {effectiveCities.map((c) => <option key={c.name} value={c.name}>{esCityName(c.name)}</option>)}
                </select>
              </div>
              <button onClick={swapCities} aria-label="Intercambiar ciudades"
              ...
              <div>
                <label className="block text-xs font-bold text-foreground uppercase tracking-wide mb-1.5">Hasta</label>
            ...
            {effectiveAvgAnnualMileageKm ? (
              <div>
                <label className="block text-xs font-bold text-foreground uppercase tracking-wide mb-1.5">Antigüedad del vehículo (opcional)</label>
                <input type="number" value={vehicleAge} onChange={(e) => setVehicleAge(e.target.value)} placeholder="ej. 5 (años)" className={iCls} />
                <p className="text-[11px] text-muted-foreground mt-1">Agrega esto para ver si el kilometraje es típico para la edad del auto.</p>
              </div>
            ) : null}

            <div className="flex items-start gap-2 text-[11px] text-muted-foreground bg-muted/20 rounded-xl p-3">
              <Info className="h-3.5 w-3.5 flex-shrink-0 mt-0.5" />
              <p>Las distancias son en línea recta (&ldquo;a vuelo de pájaro&rdquo;), no la distancia real por carretera — la distancia real suele ser un 10–25% mayor. Esta herramienta da una idea de escala, no es un planificador de rutas.</p>
            </div>
          </div>
            ...
            {result && from && to ? (
              <>
                <div className="bg-card border border-border rounded-2xl p-6">
                  <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide mb-3">Resultado</p>
                  ...
                  <p className="text-xs text-muted-foreground mb-4 pb-4 border-b border-border">
                    En números: {fmt(result.oneWayTrips, 1)} viajes de ida ({fmt(result.distanceUnit)} {unitLabel(unit)} por trayecto, línea recta) · {fmt(result.earthLaps, 1)} vueltas a la Tierra · {fmt(result.moonTrips, 2)} viajes a la Luna · {effectiveAvgAnnualMileageKm ? `${fmt(result.yearsAtAverage, 1)} años de uso típico local` : `${fmt(result.drivingHours)} horas al volante`}.
                  </p>
                  ...
                  {result.flag && (
                    <div className={`flex items-start gap-2 rounded-xl p-3 mb-4 border ${
                      ...
                      <p className={`text-xs ${result.flag === 'low' ? 'text-red-700 dark:text-red-300' : 'text-amber-700 dark:text-amber-300'}`}>
                        {result.flag === 'low'
                          ? <>Esto está muy por debajo del kilometraje típico para un auto de esta edad en {effectiveCountryName ?? 'este mercado'}. Un kilometraje inusualmente bajo puede ser genuino (uso en carretera, bien cuidado) — pero también es un patrón común de manipulación del odómetro, así que vale la pena verificarlo.</>
                          : <>Esto está muy por encima del kilometraje típico para un auto de esta edad en {effectiveCountryName ?? 'este mercado'} — no es necesariamente un problema, pero vale la pena considerarlo en el desgaste esperado y la negociación del precio.</>}
                        {vehicleCheckHref && <> <a href={vehicleCheckHref} className="underline font-semibold">Consulta el historial del VIN →</a></>}
                      </p>
                    </div>
                  )}
              ...
              <div className="bg-card border border-border rounded-2xl p-8 text-center text-sm text-muted-foreground">
                Ingresa una lectura del odómetro para ver qué significa realmente.
              </div>
            )}
          </div>
        </div>
