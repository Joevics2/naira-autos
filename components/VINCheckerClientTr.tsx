...
const VIN_LOCATIONS = [
  { n: '1', spot: 'Gösterge Paneli', detail: 'Ön camdan görülebilen, sürücü tarafındaki plaka.' },
  { n: '2', spot: 'Kapı Çerçevesi Etiketi', detail: 'Sürücü kapısının çerçevesinde — gösterge panelindeki numarayla eşleşmeli.' },
  { n: '3', spot: 'Şasiye Kazınmış', detail: 'Motor bölmesinde, şasi üzerine doğrudan kazınmış.' },
  { n: '4', spot: 'Bagaj / Koltuk Altı', detail: 'Bazı modellerde bagaj veya ön koltuk halısının altına kazınmıştır.' },
];
...
export default function VINCheckerClientTr() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('Lütfen şasi numarasını (VIN) girin.'); return; }
    if (!isValidVIN(cleaned)) { setError('Şasi numarası tam olarak 17 karakter olmalı — sadece harf (I, O, Q hariç) ve rakam.'); return; }
    ...
    } catch { setError('VIN veri tabanına bağlanılamadı. İnternet bağlantınızı kontrol edip tekrar deneyin.'); }
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="max-w-2xl mb-8">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                placeholder="Şasi numarasını / VIN'i girin (17 karakter)"
            ...
            <button onClick={decode} disabled={loading || vin.length < 17}
              ...
              {loading ? <><div className="w-4 h-4 border-2 border-muted-foreground/30 border-t-foreground rounded-full animate-spin" />Çözümleniyor...</> : <><Search className="h-4 w-4" />Çözümle</>}
            </button>
          </div>
          ...
          <div className="flex justify-between items-center mt-1">
            <span className="text-xs text-muted-foreground">{vin.length}/17 karakter</span>
            {vin.length === 17 && <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">Çözümlemeye hazır</span>}
          </div>
          {error && (
        ...
        {result ? (
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">Sınırlı veri — Kuzey Amerika dışı üretim</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">Bu şasi numarası NHTSA veri tabanında yok (Avrupa, Asya veya başka bir menşeili üretim). Model yılı, şasi numarasının yapısından hesaplandı{result.modelYear ? `: ${result.modelYear}` : ''}. Tam teknik özellikler için üreticiye başvurun.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">Şasi numarası çözümlendi — Kuzey Amerika üretimi doğrulandı</p>
                <button onClick={reset} className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> Yeni sorgu</button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(result.engineCylinders || result.engineDisplacementL || result.engineHP) && (
                <SpecCard icon={<Zap className="h-3.5 w-3.5 text-orange-500" />} title="Motor">
                  {result.engineCylinders && <Row label="Silindir" value={result.engineCylinders} />}
                  {result.engineDisplacementL && <Row label="Hacim" value={result.engineDisplacementL + 'L'} />}
                  {result.engineHP && <Row label="Güç" value={result.engineHP + ' hp'} />}
                </SpecCard>
              )}
              {(result.manufacturer || result.plantCountry) && (
                <SpecCard icon={<Globe className="h-3.5 w-3.5 text-blue-500" />} title="Menşei">
                  {result.manufacturer && <Row label="Üretici" value={result.manufacturer} />}
                  {result.plantCountry && <Row label="Ülke" value={`${getOriginFlag(result.plantCountry)} ${result.plantCountry}`} />}
                  {result.plantCity && <Row label="Fabrika" value={result.plantCity} />}
                </SpecCard>
              )}
              {(result.modelYear || result.doors || result.transmissionStyle) && (
                <SpecCard icon={<Car className="h-3.5 w-3.5 text-emerald-500" />} title="Araç">
                  {result.modelYear && <Row label="Model Yılı" value={result.modelYear} />}
                  {result.doors && <Row label="Kapı Sayısı" value={result.doors} />}
                  {result.transmissionStyle && <Row label="Vites" value={result.transmissionStyle} />}
                </SpecCard>
              )}
              {(result.abs || result.airBagLocFront) && (
                <SpecCard icon={<Shield className="h-3.5 w-3.5 text-emerald-500" />} title="Güvenlik">
                  {result.abs && <Row label="ABS" value={result.abs} />}
                  {result.airBagLocFront && <Row label="Hava Yastığı" value={result.airBagLocFront} />}
                  {result.seatBeltsAll && <Row label="Emniyet Kemeri" value={result.seatBeltsAll} />}
                </SpecCard>
              )}
            </div>

            {/* Fiziksel kontrol uyarısı */}
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">Şasi numarasını üç fiziksel noktadan kontrol edin</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Şasi Numarası Nerede Bulunur</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Şasi Numarasının Yapısı</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'Ülke ve üretici' },
                  { pos: '4–8', name: 'VDS', desc: 'Model, motor, kasa' },
                  { pos: '9', name: 'Kontrol Hanesi', desc: 'Tüm VIN\'i doğrular' },
                  { pos: '10', name: 'Model Yılı', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: 'Fabrika', desc: 'Montaj fabrikası kodu' },
                  { pos: '12–17', name: 'Seri Numarası', desc: 'Araca özel benzersiz üretim numarası' },
                ].map(({ pos, name, desc }) => (
