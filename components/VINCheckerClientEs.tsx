...
const VIN_LOCATIONS = [
  { n: '1', spot: 'Placa del tablero', detail: 'Visible a través del parabrisas, lado del conductor.' },
  { n: '2', spot: 'Etiqueta del marco de la puerta', detail: 'Dentro del marco de la puerta del conductor — debe coincidir con el tablero.' },
  { n: '3', spot: 'Grabado en el chasis', detail: 'Grabado en el riel del chasis, debajo del capó.' },
  { n: '4', spot: 'Cajuela / bajo el asiento', detail: 'Algunos modelos lo graban bajo la alfombra de la cajuela o el asiento delantero.' },
];
...
export default function VINCheckerClientEs() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('Por favor ingresa un número de VIN.'); return; }
    if (!isValidVIN(cleaned)) { setError('El VIN debe tener exactamente 17 caracteres — letras (sin I, O, Q) y números únicamente.'); return; }
    ...
    } catch { setError('No se pudo conectar con la base de datos de VIN. Revisa tu conexión e intenta de nuevo.'); }
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                placeholder="Ingresa el VIN / número de chasis (17 caracteres)"
        ...
        {result ? (
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">Este VIN está fuera de la base de datos de NHTSA (origen europeo/asiático). Año de modelo decodificado a partir de la estructura del VIN{result.modelYear ? `: ${result.modelYear}` : ''}. Para especificaciones completas, contacta al fabricante.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">VIN decodificado — vehículo de Norteamérica confirmado</p>
            ...
            {/* Advertencia de verificación del VIN */}
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">Verifica el VIN en las tres ubicaciones físicas</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href="/herramientas/mecanico-virtual" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group">
              ...
              <Link href="/tools/calculadora-de-kilometraje" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group">
                <p className="text-xs font-bold text-blue-700 dark:text-blue-400">Verifica el kilometraje</p>
                <ChevronRight className="h-3.5 w-3.5 text-blue-600 dark:text-blue-500" />
              </Link>
            </div>
          </div>
        ) : (
          /* Guía previa a la búsqueda */
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Dónde Encontrar el VIN</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Estructura del VIN</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'País y fabricante' },
                  { pos: '4–8', name: 'VDS', desc: 'Modelo, motor, carrocería' },
                  { pos: '9', name: 'Dígito verificador', desc: 'Valida el VIN completo' },
                  { pos: '10', name: 'Año del modelo', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: 'Planta', desc: 'Código de la fábrica de ensamblaje' },
