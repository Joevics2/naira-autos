...
const VIN_LOCATIONS = [
  { n: '1', spot: 'Painel do carro', detail: 'Visível através do para-brisa, do lado do motorista.' },
  { n: '2', spot: 'Etiqueta da porta', detail: 'Na moldura da porta do motorista — deve bater com o número do painel.' },
  { n: '3', spot: 'Gravado no chassi', detail: 'Gravado na longarina do chassi, embaixo do capô.' },
  { n: '4', spot: 'Porta-malas / sob o banco', detail: 'Em alguns modelos, fica gravado sob o carpete do porta-malas ou do banco dianteiro.' },
];
...
export default function VINCheckerClientPt() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('Digite o número do chassi (VIN).'); return; }
    if (!isValidVIN(cleaned)) { setError('O chassi precisa ter exatamente 17 caracteres — apenas letras (sem I, O, Q) e números.'); return; }
    ...
    } catch { setError('Não foi possível conectar à base de dados do VIN. Verifique sua conexão e tente de novo.'); }
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="max-w-2xl mb-8">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                placeholder="Digite o chassi / VIN (17 caracteres)"
          ...
          <div className="flex justify-between items-center mt-1">
            ...
            {vin.length === 17 && <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">Pronto para decodificar</span>}
          </div>
          {error && (
        ...
        {result ? (
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">Dados limitados — veículo fora da América do Norte</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">Esse chassi não está na base de dados do NHTSA (fabricação europeia, asiática, brasileira ou de outra origem). O ano do modelo foi decodificado a partir da própria estrutura do chassi{result.modelYear ? `: ${result.modelYear}` : ''}. Para especificações completas, consulte a montadora.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">Chassi decodificado — veículo confirmado da América do Norte</p>
                <button onClick={reset} className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> Nova busca</button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(result.engineCylinders || result.engineDisplacementL || result.engineHP) && (
                <SpecCard icon={<Zap className="h-3.5 w-3.5 text-orange-500" />} title="Motor">
                  ...
                  {result.engineHP && <Row label="Potência" value={result.engineHP + ' hp'} />}
                </SpecCard>
              )}
              {(result.manufacturer || result.plantCountry) && (
                <SpecCard icon={<Globe className="h-3.5 w-3.5 text-blue-500" />} title="Origem">
                  ...
                  {result.plantCity && <Row label="Fábrica" value={result.plantCity} />}
                </SpecCard>
              )}
              {(result.modelYear || result.doors || result.transmissionStyle) && (
                <SpecCard icon={<Car className="h-3.5 w-3.5 text-emerald-500" />} title="Veículo">
                  {result.modelYear && <Row label="Ano" value={result.modelYear} />}
                  {result.doors && <Row label="Portas" value={result.doors} />}
                  {result.transmissionStyle && <Row label="Câmbio" value={result.transmissionStyle} />}
                </SpecCard>
              )}
              {(result.abs || result.airBagLocFront) && (
                <SpecCard icon={<Shield className="h-3.5 w-3.5 text-emerald-500" />} title="Segurança">
                  {result.abs && <Row label="ABS" value={result.abs} />}
                  {result.airBagLocFront && <Row label="Airbags" value={result.airBagLocFront} />}
                  {result.seatBeltsAll && <Row label="Cintos" value={result.seatBeltsAll} />}
                </SpecCard>
              )}
            </div>
            ...
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">Confira o chassi nos três locais físicos</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href="/ferramentas/meu-mecanico-virtual" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group">
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">Diagnosticar esse carro</p>
                <ChevronRight className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-500" />
              </Link>
              <Link href="/ferramentas/quanto-vale-meu-carro" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group">
                <p className="text-xs font-bold text-blue-700 dark:text-blue-400">Avaliar esse carro</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Onde Encontrar o Chassi</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Estrutura do Chassi</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'País e fabricante' },
                  { pos: '4–8', name: 'VDS', desc: 'Modelo, motor, carroceria' },
                  { pos: '9', name: 'Dígito verificador', desc: 'Valida o chassi completo' },
                  { pos: '10', name: 'Ano do modelo', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: 'Fábrica', desc: 'Código da planta de montagem' },
                  { pos: '12–17', name: 'Número sequencial', desc: 'Número único de produção' },
                ].map(({ pos, name, desc }) => (
