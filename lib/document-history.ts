...
//
// Feature B: lets someone come back and reopen documents they've generated
// before — but this is real personal data (names, VINs, prices), so it is
// deliberately kept local-only, never sent to the server, and handled with
// a few guardrails:
...
const HISTORY_KEY = 'naira-autos-doc-history';
...
export function getHistory(): DocumentHistoryEntry[] {
  try {
    ...
    // Write back the pruned list so expired entries don't linger on disk.
