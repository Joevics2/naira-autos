...
export async function generateStaticParams() {
  ...
  const [{ data: parts }, { data: problems }] = await Promise.all([
    supabase.from('vehicle_parts').select('brand_slug, vehicle_type').eq('is_published', true),
    supabase.from('vehicle_problems').select('brand_slug, vehicle_type').eq('is_published', true),
  ]);
...
export default async function BrandPage(
  ...
  const [{ data: partModels }, { data: problemModels }, { data: maintenanceModels }] = await Promise.all([
    supabase.from('vehicle_parts').select('model_name, brand_name').eq('brand_slug', params.brand).eq('vehicle_type', dbType).eq('is_published', true),
    supabase.from('vehicle_problems').select('model_name, brand_name').eq('brand_slug', params.brand).eq('vehicle_type', dbType).eq('is_published', true),
    supabase.from('vehicle_maintenance').select('model_name, brand_name').eq('brand_slug', params.brand).eq('vehicle_type', dbType).eq('is_published', true),
  ]);
  ...
  const [{ data: partYears }, { data: problemYears }, { data: maintenanceYears }] = await Promise.all([
    supabase.from('vehicle_parts').select('model_name, year, image_url, image_reference').eq('brand_slug', params.brand).in('model_name', modelSlugs).eq('is_published', true),
    supabase.from('vehicle_problems').select('model_name, year').eq('brand_slug', params.brand).in('model_name', modelSlugs).eq('is_published', true),
    supabase.from('vehicle_maintenance').select('model_name, year, image_url, image_reference').eq('brand_slug', params.brand).in('model_name', modelSlugs).eq('is_published', true),
  ]);
