...
export default async function VehicleTypePage(
  ...
  const [{ data: partRows }, { data: problemRows }, { data: maintenanceRows }] = await Promise.all([
    supabase.from('vehicle_parts').select('brand_slug, brand_name, model_name').eq('vehicle_type', dbType).eq('is_published', true),
    supabase.from('vehicle_problems').select('brand_slug, brand_name, model_name').eq('vehicle_type', dbType).eq('is_published', true),
    supabase.from('vehicle_maintenance').select('brand_slug, brand_name, model_name').eq('vehicle_type', dbType).eq('is_published', true),
  ]);
