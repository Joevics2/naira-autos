...
const STORAGE_KEYS = {
  SESSIONS: 'ai_mechanic_sessions_nl',
  ACTIVE: 'ai_mechanic_active_nl',
  VEHICLE: 'ai_mechanic_vehicle_nl',
};
...
const URGENCY_CONFIG: Record<UrgencyLevel, {
  ...
  safe:         { bg: 'bg-emerald-950/40', border: 'border-emerald-700', text: 'text-emerald-300', badgeBg: 'bg-emerald-500', icon: <CheckCircle2 className="h-4 w-4 text-emerald-400" />, label: 'Veilig om te Rijden' },
  monitor:      { bg: 'bg-amber-950/40',   border: 'border-amber-700',   text: 'text-amber-300',   badgeBg: 'bg-amber-500',   icon: <AlertCircle className="h-4 w-4 text-amber-400" />, label: 'Goed in de Gaten Houden' },
  urgent:       { bg: 'bg-orange-950/40',  border: 'border-orange-700',  text: 'text-orange-300',  badgeBg: 'bg-orange-500',  icon: <AlertTriangle className="h-4 w-4 text-orange-400" />, label: 'Snel naar een Monteur' },
  stop_driving: { bg: 'bg-red-950/40',     border: 'border-red-700',     text: 'text-red-300',     badgeBg: 'bg-red-600',     icon: <XCircle className="h-4 w-4 text-red-400" />, label: 'Stop Nu met Rijden' },
};
...
const PRIORITY_CONFIG = {
  immediate:       { label: 'Nu',        cls: 'bg-red-500 text-white' },
  soon:            { label: 'Binnenkort',cls: 'bg-orange-500 text-white' },
  when_convenient: { label: 'Geen haast', cls: 'bg-emerald-500 text-white' },
};
...
function timeAgo(ts: number) {
  ...
  if (d < 3600000) return Math.floor(d / 60000) + 'm geleden';
  if (d < 86400000) return Math.floor(d / 3600000) + 'u geleden';
  return Math.floor(d / 86400000) + 'd geleden';
}

function blankSession(v: VehicleProfile): ChatSession {
  return { id: uid(), title: 'Nieuw gesprek', messages: [], vehicle: v, createdAt: Date.now(), updatedAt: Date.now() };
}
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-slate-400">Zekerheid van Axion</span>
...
function AxionLoadingSteps() {
  const steps = ['Je beschrijving lezen...', 'Storingspatronen controleren...', 'Reparatiekosten inschatten...', 'Je diagnose voorbereiden...'];
...
function DiagnosisCard({ diagnosis }: { diagnosis: DiagnosisResult }) {
  ...
  const costStr = hasCost
    ? (diagnosis.estimated_repair_cost_usd.min && diagnosis.estimated_repair_cost_usd.max
        ...
        ? 'Vanaf $' + diagnosis.estimated_repair_cost_usd.min.toLocaleString()
        : 'Tot $' + (diagnosis.estimated_repair_cost_usd.max?.toLocaleString() ?? ''))
    : null;
  ...
  return (
    <div className="mt-1 max-w-2xl rounded-2xl border border-slate-200/20 bg-slate-800 overflow-hidden">
      ...
      <div className="divide-y divide-slate-600/50">
        ...
        {diagnosis.likely_causes?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">
              {diagnosis.likely_causes.length > 1 ? 'Mogelijke oorzaken' : 'Meest waarschijnlijke oorzaak'}
            </p>
        ...
        {allActions.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Wat te doen</p>
              ...
              {allActions.map((a, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 mt-0.5 ${
                    ...
                    {a.diy ? 'Doe het zelf' : PRIORITY_CONFIG[a.priority].label}
                  </span>
        ...
        {diagnosis.next_steps_to_confirm?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-amber-400/70 uppercase tracking-wide mb-3">Voor een betere diagnose, probeer ook</p>
        ...
        {diagnosis.parts_to_check?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2.5">Te controleren onderdelen</p>
        ...
        {hasCost && costStr && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Geschatte reparatiekosten</p>
        ...
        <div className="px-4 py-3 bg-slate-700/40">
          <p className="text-xs text-slate-500 leading-relaxed">Dit is een door AI ondersteunde diagnose. Gebruik het als startpunt. Bevestig reparaties altijd bij een erkende monteur voordat je ze laat uitvoeren.</p>
        </div>
...
export default function AIMechanicClientNL() {
  ...
  const startRecording = async () => {
    try {
      ...
      alert('Microfoontoegang geweigerd. Geef toegang en probeer het opnieuw.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    const title = isFirst ? (trimmed.slice(0, 52) || 'Multimediadiagnose') : activeSession.title;
    ...
    try {
      ...
      setError(err.message || 'Kon niet analyseren. Probeer het opnieuw.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <aside className={[
            ...
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-white">Axion — Gesprekken</span>
              </div>
            ...
            <div className="p-2.5 border-b border-white/10">
              <button onClick={startNewChat}
                ...
                <Plus className="h-3.5 w-3.5" /> Nieuw gesprek
              </button>
            </div>

            <div className="flex-1 overflow-y-auto py-2 px-2">
              {sessions.filter(s => s.messages.length > 0).length === 0 ? (
                <p className="text-xs text-white/20 text-center py-8 px-3">Nog geen gesprekken</p>
                ...
                const label = thisDate === today ? 'Vandaag'
                  : new Date(s.updatedAt) > new Date(Date.now() - 86400000 * 2) ? 'Gisteren'
                  ...
                  <div key={s.id}>
                    ...
                    <div onClick={() => { setActive(s.id); setSidebarOpen(false); }}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-white/25 mt-0.5">{s.messages.length} berichten · {timeAgo(s.updatedAt)}</p>
                      </div>
            ...
            <div className="px-4 py-3 border-t border-white/10">
              <p className="text-xs text-white/20">Opgeslagen op dit apparaat.</p>
              <button onClick={() => {
                if (confirm('Alle gespreksgeschiedenis wissen?')) {
                  const s = blankSession(vehicle);
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            <div>
              <h1 className="font-black leading-tight tracking-tight text-white mb-2"
                ...
                Hoi, ik ben Axion 👋 jouw AI-monteur.
              </h1>
              ...
              {!hasMessages && (
                <details className="mt-3 mb-5 group">
                  <summary className="flex items-center gap-2 cursor-pointer list-none text-xs text-white/40 hover:text-white/60 transition-colors w-fit">
                    ...
                    <span className="font-semibold uppercase tracking-wide">Hoe het werkt</span>
                  </summary>
                  <div className="flex flex-col sm:flex-row gap-3 mt-3">
                    {[
                      { num: '1', text: 'Beschrijf, neem geluid op of film het probleem met je auto — elk detail helpt.' },
                      { num: '2', text: 'Upload of beschrijf het probleem in het tekstvak hieronder.' },
                      { num: '3', text: 'Ontvang direct een diagnose, kosteninschatting en vervolgstappen.' },
                    ].map(({ num, text: t }) => (
            ...
            {!hasMessages && (
              <div>
                ...
                <div className="flex gap-2 mb-1.5">
                  <div className="relative flex-1 min-w-0">
                    <select value={vehicle.brand} onChange={e => persistVehicle({ ...vehicle, brand: e.target.value })}
                      ...
                      <option value="" style={{ background: '#080C10' }}>Merk (optioneel)</option>
                      {NIGERIAN_BRANDS.map(b => <option key={b} value={b} style={{ background: '#080C10' }}>{b}</option>)}
                    </select>
                  </div>
                  <input type="text" placeholder="Model (optioneel)" value={vehicle.model}
                  ...
                  <input type="number" placeholder="Bouwjaar (optioneel)" value={vehicle.year}
                ...
                <p className="text-xs text-white/30 mb-2">Optioneel, maar hoe meer Axion over je auto weet, hoe nauwkeuriger de diagnose.</p>
                ...
                <div className="border border-white/20 rounded-xl bg-white/10 overflow-hidden focus-within:border-emerald-500/50 transition-all">
                  <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                    placeholder="Beschrijf het probleem met je auto... bijv. tikkend geluid bij koude start, motorstoringslampje aan, slappe remmen"
                    rows={3}
                  ...
                  <div className="flex items-center gap-2 px-3 pb-2 pt-1.5 border-t border-white/20 flex-wrap">
                    ...
                    {showAudioUpload && <MediaPill icon={<Mic className="h-3 w-3" />} label="Geluid" accept="audio/*" file={audioFile} onFile={setAudioFile} onClear={() => setAudioFile(null)} maxMB={20} />}
                  ...
                  <div className="px-3 pb-3">
                    <button onClick={handleSubmit} disabled={!hasInput || loading}
                      ...
                      {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Analyseren...</> : <><Wrench className="h-4 w-4" /> Diagnosticeren</>}
                    </button>
                  </div>
                </div>
                ...
                {vehicleSummary && (
                  <p className="text-xs text-white/25 mt-2 flex items-center gap-1.5">
                    <Check className="h-3 w-3 text-emerald-500" /> Voertuig opgeslagen op dit apparaat
                  </p>
                )}
              </div>
            )}
              ...
              <div className="bg-[#0D1117]/80 border border-white/10 rounded-2xl overflow-hidden flex flex-col backdrop-blur-sm"
                ...
                <div className="border-t border-white/10 bg-[#080C10]/80 px-3 py-2.5">
                  <div className="flex gap-2 items-center">
                    <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                      placeholder="Stel een vervolgvraag... (Enter om te versturen)" rows={1}
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <button onClick={startNewChat} className="flex items-center gap-1 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                      <Plus className="h-3 w-3" /> Nieuw gesprek
                    </button>
                  </div>
                </div>
              </div>
            ...
            {!hasMessages && (
              <div className="hidden lg:flex items-center justify-end">
                <button onClick={startNewChat} className="flex items-center gap-1.5 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                  <Plus className="h-3 w-3" /> Start een nieuw gesprek
                </button>
              </div>
            )}
