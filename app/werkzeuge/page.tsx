...
export const metadata: Metadata = {
  ...
  description: 'Kostenlose Werkzeuge für Ihr Auto auf Deutsch — virtueller KI-Mechaniker, und weitere Werkzeuge folgen bald. Alles kostenlos und ohne Registrierung.',
  alternates: {
    canonical: 'https://www.naira.autos/werkzeuge',
    languages: {
  ...
  keywords: ['kostenlose Auto-Werkzeuge', 'virtueller Mechaniker kostenlos', 'Fahrzeug-Werkzeuge auf Deutsch', 'KI-Mechaniker'],
};

const CATEGORY_COLORS: Record<string, string> = {
  'KI und intelligente Werkzeuge': 'text-emerald-600 dark:text-emerald-400',
  'Finanzen': 'text-sky-600 dark:text-sky-400',
  'Kosten und Wartung': 'text-yellow-600 dark:text-yellow-400',
  'Überprüfung': 'text-rose-600 dark:text-rose-400',
  'Ressourcen': 'text-violet-600 dark:text-violet-400',
};

const CATEGORY_BORDER: Record<string, string> = {
  'KI und intelligente Werkzeuge': 'hover:border-emerald-500/40 hover:shadow-emerald-500/5',
  'Finanzen': 'hover:border-sky-500/40 hover:shadow-sky-500/5',
  'Kosten und Wartung': 'hover:border-yellow-500/40 hover:shadow-yellow-500/5',
  'Überprüfung': 'hover:border-rose-500/40 hover:shadow-rose-500/5',
  'Ressourcen': 'hover:border-violet-500/40 hover:shadow-violet-500/5',
};

const ICON_BG: Record<string, string> = {
  'KI und intelligente Werkzeuge': 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400',
  'Finanzen': 'bg-sky-500/10 text-sky-600 dark:text-sky-400',
  'Kosten und Wartung': 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400',
  'Überprüfung': 'bg-rose-500/10 text-rose-600 dark:text-rose-400',
  'Ressourcen': 'bg-violet-500/10 text-violet-600 dark:text-violet-400',
};

export default function ToolsGermanPage() {
  const liveCategories = CATEGORIES_DE.filter((c) => TOOLS_DE.some((t) => t.category === c));
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-[#080C10] pt-16 pb-14 px-4">
        <div className="max-w-screen-xl mx-auto">
          ...
          >
            Werkzeug-<br />
            <span className="text-emerald-400">Zentrale</span>
          </h1>
          <p className="text-white/50 text-base md:text-lg font-light max-w-lg leading-relaxed">
            Alles, was Sie zum Kaufen, Verkaufen und Pflegen Ihres Autos brauchen — kostenlos, ohne Registrierung.
          </p>
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-12 space-y-14">
        {liveCategories.map((category) => {
          const categoryTools = TOOLS_DE.filter(t => t.category === category);
          return (
            <div key={category}>
