...
const STORAGE_KEYS = {
  SESSIONS: 'ai_mechanic_sessions_tr',
  ACTIVE: 'ai_mechanic_active_tr',
  VEHICLE: 'ai_mechanic_vehicle_tr',
};
...
const URGENCY_CONFIG: Record<UrgencyLevel, {
  ...
  safe:         { bg: 'bg-emerald-950/40', border: 'border-emerald-700', text: 'text-emerald-300', badgeBg: 'bg-emerald-500', icon: <CheckCircle2 className="h-4 w-4 text-emerald-400" />, label: 'Sürüş İçin Güvenli' },
  monitor:      { bg: 'bg-amber-950/40',   border: 'border-amber-700',   text: 'text-amber-300',   badgeBg: 'bg-amber-500',   icon: <AlertCircle className="h-4 w-4 text-amber-400" />, label: 'Yakından İzle' },
  urgent:       { bg: 'bg-orange-950/40',  border: 'border-orange-700',  text: 'text-orange-300',  badgeBg: 'bg-orange-500',  icon: <AlertTriangle className="h-4 w-4 text-orange-400" />, label: 'Yakında Bir Ustaya Görün' },
  stop_driving: { bg: 'bg-red-950/40',     border: 'border-red-700',     text: 'text-red-300',     badgeBg: 'bg-red-600',     icon: <XCircle className="h-4 w-4 text-red-400" />, label: 'Şimdi Sürmeyi Bırak' },
};
...
const PRIORITY_CONFIG = {
  immediate:       { label: 'Şimdi',     cls: 'bg-red-500 text-white' },
  soon:            { label: 'Yakında',   cls: 'bg-orange-500 text-white' },
  when_convenient: { label: 'Acelesi Yok', cls: 'bg-emerald-500 text-white' },
};
...
function blankSession(v: VehicleProfile): ChatSession {
  return { id: uid(), title: 'Yeni konuşma', messages: [], vehicle: v, createdAt: Date.now(), updatedAt: Date.now() };
}
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-slate-400">Axion Güven Oranı</span>
    ...
    <>
      <input ref={ref} type="file" accept={accept} className="hidden"
        onChange={e => {
          ...
          if (!f) return;
          if (f.size > maxMB * 1024 * 1024) { alert('Maks ' + maxMB + 'MB'); return; }
...
function AxionLoadingSteps() {
  const steps = ['Açıklamanı okuyorum...', 'Arıza kalıplarını kontrol ediyorum...', 'Tamir maliyetini hesaplıyorum...', 'Teşhisini hazırlıyorum...'];
...
function DiagnosisCard({ diagnosis }: { diagnosis: DiagnosisResult }) {
  ...
  const costStr = hasCost
    ? (diagnosis.estimated_repair_cost_usd.min && diagnosis.estimated_repair_cost_usd.max
        ...
        ? 'En az $' + diagnosis.estimated_repair_cost_usd.min.toLocaleString()
        : 'En fazla $' + (diagnosis.estimated_repair_cost_usd.max?.toLocaleString() ?? ''))
    : null;
  ...
  return (
    <div className="mt-1 max-w-2xl rounded-2xl border border-slate-200/20 bg-slate-800 overflow-hidden">
      ...
      <div className="divide-y divide-slate-600/50">
        ...
        {diagnosis.likely_causes?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">
              {diagnosis.likely_causes.length > 1 ? 'Olası Nedenler' : 'En Olası Neden'}
            </p>
        ...
        {allActions.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Ne Yapılmalı</p>
              ...
              {allActions.map((a, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 mt-0.5 ${
                    ...
                    {a.diy ? 'Kendin Yap' : PRIORITY_CONFIG[a.priority].label}
                  </span>
        ...
        {diagnosis.next_steps_to_confirm?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-amber-400/70 uppercase tracking-wide mb-3">Daha iyi bir teşhis için ayrıca dene</p>
        ...
        {diagnosis.parts_to_check?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2.5">Kontrol Edilecek Parçalar</p>
        ...
        {hasCost && costStr && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Tahmini Tamir Maliyeti</p>
        ...
        <div className="px-4 py-3 bg-slate-700/40">
          <p className="text-xs text-slate-500 leading-relaxed">Bu, yapay zeka destekli bir teşhistir. Başlangıç noktası olarak kullan. Tamirat yapmadan önce her zaman yetkili bir ustaya danış.</p>
        </div>
...
export default function AIMechanicClientTR() {
  ...
  const startRecording = async () => {
    try {
      ...
      alert('Mikrofon erişimi reddedildi. Erişime izin verip tekrar dene.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    const title = isFirst ? (trimmed.slice(0, 52) || 'Medya teşhisi') : activeSession.title;
    ...
    try {
      ...
      setError(err.message || 'Analiz edilemedi. Tekrar dene.');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <div className="relative z-10 max-w-screen-xl mx-auto px-4 sm:px-6 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-8">
          <Link href="/araclar" className="flex items-center justify-center w-8 h-8 rounded-full bg-white/10 hover:bg-emerald-500/20 border border-white/15 hover:border-emerald-500/40 text-white/60 hover:text-emerald-400 transition-all" aria-label="Araçlara Dön">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-white/30">
            ...
            <ChevronRight className="h-3 w-3" />
            <span className="text-white/50">Yapay Zeka Ustası</span>
          </nav>
          ...
          <aside className={[
            ...
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-white">Axion — Sohbetler</span>
              </div>
            ...
            <div className="p-2.5 border-b border-white/10">
              <button onClick={startNewChat}
                ...
                <Plus className="h-3.5 w-3.5" /> Yeni konuşma
              </button>
            </div>

            <div className="flex-1 overflow-y-auto py-2 px-2">
              {sessions.filter(s => s.messages.length > 0).length === 0 ? (
                <p className="text-xs text-white/20 text-center py-8 px-3">Henüz konuşma yok</p>
                ...
                const label = thisDate === today ? 'Bugün'
                  : new Date(s.updatedAt) > new Date(Date.now() - 86400000 * 2) ? 'Dün'
                  ...
                  <div key={s.id}>
                    ...
                    <div onClick={() => { setActive(s.id); setSidebarOpen(false); }}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-white/25 mt-0.5">{s.messages.length} mesaj · {timeAgo(s.updatedAt)}</p>
                      </div>
            ...
            <div className="px-4 py-3 border-t border-white/10">
              <p className="text-xs text-white/20">Bu cihazda saklanıyor.</p>
              <button onClick={() => {
                if (confirm('Tüm konuşma geçmişi silinsin mi?')) {
                  const s = blankSession(vehicle);
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            <div>
              <h1 className="font-black leading-tight tracking-tight text-white mb-2"
                ...
                Merhaba, ben Axion 👋 yapay zeka ustan.
              </h1>
              ...
              {!hasMessages && (
                <details className="mt-3 mb-5 group">
                  <summary className="flex items-center gap-2 cursor-pointer list-none text-xs text-white/40 hover:text-white/60 transition-colors w-fit">
                    ...
                    <span className="font-semibold uppercase tracking-wide">Nasıl çalışır</span>
                  </summary>
                  <div className="flex flex-col sm:flex-row gap-3 mt-3">
                    {[
                      { num: '1', text: 'Arabanın sorununu anlat, sesini kaydet ya da videosunu çek — her detay yardımcı olur.' },
                      { num: '2', text: 'Aşağıdaki kutuyu kullanarak yükle ya da sorunu anlat.' },
                      { num: '3', text: 'Anında teşhis, tahmini maliyet ve sonraki adımları öğren.' },
                    ].map(({ num, text: t }) => (
            ...
            {!hasMessages && (
              <div>
                ...
                <div className="flex gap-2 mb-1.5">
                  <div className="relative flex-1 min-w-0">
                    <select value={vehicle.brand} onChange={e => persistVehicle({ ...vehicle, brand: e.target.value })}
                      ...
                      <option value="" style={{ background: '#080C10' }}>Marka (opsiyonel)</option>
                      {NIGERIAN_BRANDS.map(b => <option key={b} value={b} style={{ background: '#080C10' }}>{b}</option>)}
                    </select>
                  </div>
                  <input type="text" placeholder="Model (opsiyonel)" value={vehicle.model}
                  ...
                  <input type="number" placeholder="Yıl (opsiyonel)" value={vehicle.year}
                ...
                <p className="text-xs text-white/30 mb-2">Opsiyonel, ama Axion arabanı ne kadar iyi tanırsa teşhis o kadar isabetli olur.</p>
                ...
                <div className="border border-white/20 rounded-xl bg-white/10 overflow-hidden focus-within:border-emerald-500/50 transition-all">
                  <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                    placeholder="Arabanın sorununu anlat... örn. soğuk çalıştırmada vuruntu, motor arıza lambası yanıyor, fren yumuşak"
                    rows={3}
                  ...
                  <div className="flex items-center gap-2 px-3 pb-2 pt-1.5 border-t border-white/20 flex-wrap">
                    {showImageUpload && <MediaPill icon={<Camera className="h-3 w-3" />} label="Fotoğraf" accept="image/*" file={imageFile} onFile={setImageFile} onClear={() => setImageFile(null)} maxMB={10} />}
                    {showAudioUpload && <MediaPill icon={<Mic className="h-3 w-3" />} label="Ses" accept="audio/*" file={audioFile} onFile={setAudioFile} onClear={() => setAudioFile(null)} maxMB={20} />}
                  ...
                  <div className="px-3 pb-3">
                    <button onClick={handleSubmit} disabled={!hasInput || loading}
                      ...
                      {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Analiz ediliyor...</> : <><Wrench className="h-4 w-4" /> Teşhis Et</>}
                    </button>
                  </div>
                </div>
                ...
                {vehicleSummary && (
                  <p className="text-xs text-white/25 mt-2 flex items-center gap-1.5">
                    <Check className="h-3 w-3 text-emerald-500" /> Araç bu cihaza kaydedildi
                  </p>
                )}
              </div>
            )}
              ...
              <div className="bg-[#0D1117]/80 border border-white/10 rounded-2xl overflow-hidden flex flex-col backdrop-blur-sm"
                ...
                <div className="border-t border-white/10 bg-[#080C10]/80 px-3 py-2.5">
                  <div className="flex gap-2 items-center">
                    <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                      placeholder="Bir takip sorusu sor... (Göndermek için Enter'a bas)" rows={1}
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <button onClick={startNewChat} className="flex items-center gap-1 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                      <Plus className="h-3 w-3" /> Yeni sohbet
                    </button>
                  </div>
                </div>
              </div>
            ...
            {!hasMessages && (
              <div className="hidden lg:flex items-center justify-end">
                <button onClick={startNewChat} className="flex items-center gap-1.5 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                  <Plus className="h-3 w-3" /> Yeni bir konuşma başlat
                </button>
              </div>
            )}
