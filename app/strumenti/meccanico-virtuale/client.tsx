...
const STORAGE_KEYS = {
  SESSIONS: 'ai_mechanic_sessions_it',
  ACTIVE: 'ai_mechanic_active_it',
  VEHICLE: 'ai_mechanic_vehicle_it',
};
...
const NIGERIAN_BRANDS = [
  ...
  'MG', 'Chery', 'BYD', 'Haval', 'Innoson', 'Altro',
];
...
const URGENCY_CONFIG: Record<UrgencyLevel, {
  ...
  safe:         { bg: 'bg-emerald-950/40', border: 'border-emerald-700', text: 'text-emerald-300', badgeBg: 'bg-emerald-500', icon: <CheckCircle2 className="h-4 w-4 text-emerald-400" />, label: 'Sicuro da guidare' },
  monitor:      { bg: 'bg-amber-950/40',   border: 'border-amber-700',   text: 'text-amber-300',   badgeBg: 'bg-amber-500',   icon: <AlertCircle className="h-4 w-4 text-amber-400" />, label: 'Da tenere sotto controllo' },
  urgent:       { bg: 'bg-orange-950/40',  border: 'border-orange-700',  text: 'text-orange-300',  badgeBg: 'bg-orange-500',  icon: <AlertTriangle className="h-4 w-4 text-orange-400" />, label: 'Vai presto dal meccanico' },
  stop_driving: { bg: 'bg-red-950/40',     border: 'border-red-700',     text: 'text-red-300',     badgeBg: 'bg-red-600',     icon: <XCircle className="h-4 w-4 text-red-400" />, label: 'Smetti subito di guidare' },
};
...
const PROB_LABELS: Record<'high' | 'medium' | 'low', string> = {
  high: 'Probabilità alta',
  medium: 'Probabilità media',
  low: 'Probabilità bassa',
};

const PRIORITY_CONFIG = {
  immediate:       { label: 'Subito',        cls: 'bg-red-500 text-white' },
  soon:            { label: 'Presto',        cls: 'bg-orange-500 text-white' },
  when_convenient: { label: 'Senza fretta',  cls: 'bg-emerald-500 text-white' },
};
...
function timeAgo(ts: number) {
  const d = Date.now() - ts;
  if (d < 60000) return 'Proprio ora';
  if (d < 3600000) return Math.floor(d / 60000) + 'min fa';
  if (d < 86400000) return Math.floor(d / 3600000) + 'h fa';
  return Math.floor(d / 86400000) + 'g fa';
}

function blankSession(v: VehicleProfile): ChatSession {
  return { id: uid(), title: 'Nuova conversazione', messages: [], vehicle: v, createdAt: Date.now(), updatedAt: Date.now() };
}
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-slate-400">Grado di sicurezza di Axion</span>
...
function AxionLoadingSteps() {
  const steps = ['Lettura della descrizione...', 'Analisi dei pattern di guasto...', 'Stima del costo di riparazione...', 'Preparazione della diagnosi...'];
...
function DiagnosisCard({ diagnosis }: { diagnosis: DiagnosisResult }) {
  ...
  const costStr = hasCost
    ? (diagnosis.estimated_repair_cost_usd.min && diagnosis.estimated_repair_cost_usd.max
        ...
        ? 'Da $' + diagnosis.estimated_repair_cost_usd.min.toLocaleString()
        : 'Fino a $' + (diagnosis.estimated_repair_cost_usd.max?.toLocaleString() ?? ''))
    : null;
  ...
  return (
    <div className="mt-1 max-w-2xl rounded-2xl border border-slate-200/20 bg-slate-800 overflow-hidden">
      ...
      <div className="divide-y divide-slate-600/50">
        ...
        {diagnosis.likely_causes?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">
              {diagnosis.likely_causes.length > 1 ? 'Possibili cause' : 'Causa più probabile'}
            </p>
        ...
        {allActions.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Cosa fare</p>
              ...
              {allActions.map((a, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 mt-0.5 whitespace-nowrap ${
                    ...
                    {a.diy ? 'Puoi farlo da solo' : PRIORITY_CONFIG[a.priority].label}
                  </span>
        ...
        {diagnosis.next_steps_to_confirm?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-amber-400/70 uppercase tracking-wide mb-3">Per una diagnosi più precisa, prova anche</p>
        ...
        {diagnosis.parts_to_check?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2.5">Parti da controllare</p>
        ...
        {hasCost && costStr && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Costo di riparazione stimato</p>
        ...
        <div className="px-4 py-3 bg-slate-700/40">
          <p className="text-xs text-slate-500 leading-relaxed">Questa è una diagnosi assistita dall'IA. Usala come punto di partenza e conferma sempre con un professionista qualificato prima di qualsiasi riparazione, soprattutto per freni, sterzo o impianto carburante.</p>
        </div>
...
export default function AIMechanicClientIT() {
  ...
  const startRecording = async () => {
    try {
      ...
      alert("Accesso al microfono negato. Consenti l'accesso e riprova.");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    const title = isFirst ? (trimmed.slice(0, 52) || 'Diagnosi multimediale') : activeSession.title;
    ...
    try {
      ...
      setError(err.message || 'Impossibile analizzare. Riprova.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <aside className={[
            ...
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-white">Axion — Conversazioni</span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto py-2 px-2">
              {sessions.filter(s => s.messages.length > 0).length === 0 ? (
                <p className="text-xs text-white/20 text-center py-8 px-3">Nessuna conversazione ancora</p>
                ...
                const label = thisDate === today ? 'Oggi'
                  : new Date(s.updatedAt) > new Date(Date.now() - 86400000 * 2) ? 'Ieri'
                  ...
                  <div key={s.id}>
                    ...
                    <div onClick={() => { setActive(s.id); setSidebarOpen(false); }}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-white/25 mt-0.5">{s.messages.length} msg · {timeAgo(s.updatedAt)}</p>
                      </div>
            ...
            <div className="px-4 py-3 border-t border-white/10">
              <p className="text-xs text-white/20">Salvato su questo dispositivo.</p>
              <button onClick={() => {
                if (confirm('Cancellare tutta la cronologia delle conversazioni?')) {
                  const s = blankSession(vehicle);
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
              ...
              {!hasMessages && (
                <details className="mt-3 mb-5 group">
                  <summary className="flex items-center gap-2 cursor-pointer list-none text-xs text-white/40 hover:text-white/60 transition-colors w-fit">
                    ...
                    <span className="font-semibold uppercase tracking-wide">Come funziona</span>
                  </summary>
                  <div className="flex flex-col sm:flex-row gap-3 mt-3">
                    {[
                      { num: '1', text: 'Descrivi, registra o filma il problema della tua auto — ogni dettaglio aiuta.' },
                      { num: '2', text: 'Carica il file o descrivi il problema nel campo qui sotto.' },
                      { num: '3', text: 'Ricevi subito una diagnosi, il costo stimato e i prossimi passi.' },
                    ].map(({ num, text: t }) => (
            ...
            {!hasMessages && (
              <div>
                ...
                <div className="flex gap-2 mb-1.5">
                  <div className="relative flex-1 min-w-0">
                    <select value={vehicle.brand} onChange={e => persistVehicle({ ...vehicle, brand: e.target.value })}
                      ...
                      <option value="" style={{ background: '#080C10' }}>Marca (opzionale)</option>
                      {NIGERIAN_BRANDS.map(b => <option key={b} value={b} style={{ background: '#080C10' }}>{b}</option>)}
                    </select>
                  </div>
                  <input type="text" placeholder="Modello (opzionale)" value={vehicle.model}
                  ...
                  <input type="number" placeholder="Anno" value={vehicle.year}
                ...
                <p className="text-xs text-white/30 mb-2">Opzionale, ma più Axion sa della tua auto, più precisa sarà la diagnosi.</p>
                ...
                <div className="border border-white/20 rounded-xl bg-white/10 overflow-hidden focus-within:border-emerald-500/50 transition-all">
                  <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                    placeholder="Descrivi il problema della tua auto... es. rumore metallico all'avviamento a freddo, spia motore accesa, freni molli"
                    rows={3}
                  ...
                  <div className="flex items-center gap-2 px-3 pb-2 pt-1.5 border-t border-white/20 flex-wrap">
                    ...
                    {showAudioUpload && <MediaPill icon={<Mic className="h-3 w-3" />} label="Audio" accept="audio/*" file={audioFile} onFile={setAudioFile} onClear={() => setAudioFile(null)} maxMB={20} />}
                  ...
                  <div className="px-3 pb-3">
                    <button onClick={handleSubmit} disabled={!hasInput || loading}
                      ...
                      {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Analisi in corso...</> : <><Wrench className="h-4 w-4" /> Diagnostica</>}
                    </button>
                  </div>
                </div>
                ...
                {vehicleSummary && (
                  <p className="text-xs text-white/25 mt-2 flex items-center gap-1.5">
                    <Check className="h-3 w-3 text-emerald-500" /> Veicolo salvato su questo dispositivo
                  </p>
                )}
              </div>
            )}
              ...
              <div className="bg-[#0D1117]/80 border border-white/10 rounded-2xl overflow-hidden flex flex-col backdrop-blur-sm"
                ...
                <div className="border-t border-white/10 bg-[#080C10]/80 px-3 py-2.5">
                  <div className="flex gap-2 items-center">
                    <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                      placeholder="Fai una domanda di follow-up... (Invio per inviare)" rows={1}
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <button onClick={startNewChat} className="flex items-center gap-1 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                      <Plus className="h-3 w-3" /> Nuova conversazione
                    </button>
                  </div>
                </div>
              </div>
            ...
            {!hasMessages && (
              <div className="hidden lg:flex items-center justify-end">
                <button onClick={startNewChat} className="flex items-center gap-1.5 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                  <Plus className="h-3 w-3" /> Inizia una nuova conversazione
                </button>
              </div>
            )}
