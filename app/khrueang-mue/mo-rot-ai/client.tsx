...
const STORAGE_KEYS = {
  SESSIONS: 'ai_mechanic_sessions_th',
  ACTIVE: 'ai_mechanic_active_th',
  VEHICLE: 'ai_mechanic_vehicle_th',
};
...
const URGENCY_CONFIG: Record<UrgencyLevel, {
  ...
  safe:         { bg: 'bg-emerald-950/40', border: 'border-emerald-700', text: 'text-emerald-300', badgeBg: 'bg-emerald-500', icon: <CheckCircle2 className="h-4 w-4 text-emerald-400" />, label: 'ขับได้อย่างปลอดภัย' },
  monitor:      { bg: 'bg-amber-950/40',   border: 'border-amber-700',   text: 'text-amber-300',   badgeBg: 'bg-amber-500',   icon: <AlertCircle className="h-4 w-4 text-amber-400" />, label: 'เฝ้าระวังอย่างใกล้ชิด' },
  urgent:       { bg: 'bg-orange-950/40',  border: 'border-orange-700',  text: 'text-orange-300',  badgeBg: 'bg-orange-500',  icon: <AlertTriangle className="h-4 w-4 text-orange-400" />, label: 'ควรพบช่างโดยเร็ว' },
  stop_driving: { bg: 'bg-red-950/40',     border: 'border-red-700',     text: 'text-red-300',     badgeBg: 'bg-red-600',     icon: <XCircle className="h-4 w-4 text-red-400" />, label: 'หยุดขับทันที' },
};
...
const PRIORITY_CONFIG = {
  immediate:       { label: 'ตอนนี้',     cls: 'bg-red-500 text-white' },
  soon:            { label: 'เร็ว ๆ นี้', cls: 'bg-orange-500 text-white' },
  when_convenient: { label: 'ไม่เร่งด่วน', cls: 'bg-emerald-500 text-white' },
};
...
function timeAgo(ts: number) {
  ...
  if (d < 86400000) return Math.floor(d / 3600000) + ' ชม. ที่แล้ว';
  return Math.floor(d / 86400000) + ' วันที่แล้ว';
}

function blankSession(v: VehicleProfile): ChatSession {
  return { id: uid(), title: 'การสนทนาใหม่', messages: [], vehicle: v, createdAt: Date.now(), updatedAt: Date.now() };
}
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-slate-400">ความมั่นใจของ Axion</span>
    ...
    <>
      <input ref={ref} type="file" accept={accept} className="hidden"
        onChange={e => {
          ...
          if (!f) return;
          if (f.size > maxMB * 1024 * 1024) { alert('สูงสุด ' + maxMB + 'MB'); return; }
...
function AxionLoadingSteps() {
  const steps = ['กำลังอ่านคำอธิบายของคุณ...', 'กำลังตรวจสอบรูปแบบความเสียหาย...', 'กำลังประเมินค่าซ่อม...', 'กำลังเตรียมผลวินิจฉัยของคุณ...'];
...
function DiagnosisCard({ diagnosis }: { diagnosis: DiagnosisResult }) {
  ...
  const costStr = hasCost
    ? (diagnosis.estimated_repair_cost_usd.min && diagnosis.estimated_repair_cost_usd.max
        ...
        ? 'เริ่มต้น $' + diagnosis.estimated_repair_cost_usd.min.toLocaleString()
        : 'สูงสุด $' + (diagnosis.estimated_repair_cost_usd.max?.toLocaleString() ?? ''))
    : null;
  ...
  return (
    <div className="mt-1 max-w-2xl rounded-2xl border border-slate-200/20 bg-slate-800 overflow-hidden">
      ...
      <div className="divide-y divide-slate-600/50">
        ...
        {diagnosis.likely_causes?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">
              {diagnosis.likely_causes.length > 1 ? 'สาเหตุที่เป็นไปได้' : 'สาเหตุที่เป็นไปได้มากที่สุด'}
            </p>
        ...
        {allActions.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">สิ่งที่ควรทำ</p>
              ...
              {allActions.map((a, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 mt-0.5 ${
                    ...
                    {a.diy ? 'ทำเองได้' : PRIORITY_CONFIG[a.priority].label}
                  </span>
        ...
        {diagnosis.next_steps_to_confirm?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-amber-400/70 uppercase tracking-wide mb-3">เพื่อผลวินิจฉัยที่ดีขึ้น ลองทำสิ่งนี้ด้วย</p>
        ...
        {diagnosis.parts_to_check?.length > 0 && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2.5">ชิ้นส่วนที่ควรตรวจสอบ</p>
        ...
        {hasCost && costStr && (
          <div className="px-4 py-3.5">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">ค่าซ่อมโดยประมาณ</p>
        ...
        <div className="px-4 py-3 bg-slate-700/40">
          <p className="text-xs text-slate-500 leading-relaxed">นี่คือการวินิจฉัยด้วยความช่วยเหลือของ AI ใช้เป็นจุดเริ่มต้นเท่านั้น ควรตรวจสอบกับช่างที่มีความชำนาญเสมอก่อนทำการซ่อม</p>
        </div>
...
export default function AIMechanicClientTH() {
  ...
  const startRecording = async () => {
    try {
      ...
      alert('การเข้าถึงไมโครโฟนถูกปฏิเสธ กรุณาอนุญาตแล้วลองใหม่อีกครั้ง');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    const title = isFirst ? (trimmed.slice(0, 52) || 'การวินิจฉัยจากสื่อมัลติมีเดีย') : activeSession.title;
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'เกิดข้อผิดพลาดบางอย่าง');
      ...
      setError(err.message || 'ไม่สามารถวิเคราะห์ได้ กรุณาลองใหม่');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <div className="relative z-10 max-w-screen-xl mx-auto px-4 sm:px-6 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-8">
          <Link href="/khrueang-mue" className="flex items-center justify-center w-8 h-8 rounded-full bg-white/10 hover:bg-emerald-500/20 border border-white/15 hover:border-emerald-500/40 text-white/60 hover:text-emerald-400 transition-all" aria-label="กลับไปที่เครื่องมือ">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-white/30">
            ...
            <ChevronRight className="h-3 w-3" />
            <span className="text-white/50">หมอรถ AI</span>
          </nav>
          ...
          <aside className={[
            ...
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-white">Axion — แชท</span>
              </div>
            ...
            <div className="p-2.5 border-b border-white/10">
              <button onClick={startNewChat}
                ...
                <Plus className="h-3.5 w-3.5" /> การสนทนาใหม่
              </button>
            </div>

            <div className="flex-1 overflow-y-auto py-2 px-2">
              {sessions.filter(s => s.messages.length > 0).length === 0 ? (
                <p className="text-xs text-white/20 text-center py-8 px-3">ยังไม่มีการสนทนา</p>
                ...
                const label = thisDate === today ? 'วันนี้'
                  : new Date(s.updatedAt) > new Date(Date.now() - 86400000 * 2) ? 'เมื่อวาน'
                  ...
                  <div key={s.id}>
                    ...
                    <div onClick={() => { setActive(s.id); setSidebarOpen(false); }}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-white/25 mt-0.5">{s.messages.length} ข้อความ · {timeAgo(s.updatedAt)}</p>
                      </div>
            ...
            <div className="px-4 py-3 border-t border-white/10">
              <p className="text-xs text-white/20">บันทึกไว้ในอุปกรณ์นี้</p>
              <button onClick={() => {
                if (confirm('ลบประวัติการสนทนาทั้งหมดหรือไม่?')) {
                  const s = blankSession(vehicle);
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            <div>
              <h1 className="font-black leading-tight tracking-tight text-white mb-2"
                ...
                สวัสดี ผมคือ Axion 👋 หมอรถ AI ของคุณ
              </h1>
              ...
              {!hasMessages && (
                <details className="mt-3 mb-5 group">
                  <summary className="flex items-center gap-2 cursor-pointer list-none text-xs text-white/40 hover:text-white/60 transition-colors w-fit">
                    ...
                    <span className="font-semibold uppercase tracking-wide">วิธีใช้งาน</span>
                  </summary>
                  <div className="flex flex-col sm:flex-row gap-3 mt-3">
                    {[
                      { num: '1', text: 'อธิบาย อัดเสียง หรือถ่ายวิดีโอปัญหาของรถคุณ — รายละเอียดทุกอย่างช่วยได้' },
                      { num: '2', text: 'อัปโหลดหรือเล่าปัญหาโดยใช้ช่องข้อความด้านล่าง' },
                      { num: '3', text: 'รับผลวินิจฉัยทันที ค่าใช้จ่ายโดยประมาณ และขั้นตอนถัดไป' },
                    ].map(({ num, text: t }) => (
            ...
            {!hasMessages && (
              <div>
                ...
                <div className="flex gap-2 mb-1.5">
                  <div className="relative flex-1 min-w-0">
                    <select value={vehicle.brand} onChange={e => persistVehicle({ ...vehicle, brand: e.target.value })}
                      ...
                      <option value="" style={{ background: '#080C10' }}>ยี่ห้อ (ไม่บังคับ)</option>
                      {NIGERIAN_BRANDS.map(b => <option key={b} value={b} style={{ background: '#080C10' }}>{b}</option>)}
                    </select>
                  </div>
                  <input type="text" placeholder="รุ่น (ไม่บังคับ)" value={vehicle.model}
                  ...
                  <input type="number" placeholder="ปีที่ผลิต (ไม่บังคับ)" value={vehicle.year}
                ...
                <p className="text-xs text-white/30 mb-2">ไม่บังคับ แต่ยิ่ง Axion รู้เกี่ยวกับรถของคุณมากเท่าไหร่ การวินิจฉัยก็จะยิ่งแม่นยำมากขึ้น</p>
                ...
                <div className="border border-white/20 rounded-xl bg-white/10 overflow-hidden focus-within:border-emerald-500/50 transition-all">
                  <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                    placeholder="อธิบายปัญหาของรถคุณ... เช่น เสียงเคาะตอนสตาร์ทเครื่องเย็น ไฟ check engine ติด เบรกนุ่มผิดปกติ"
                    rows={3}
                  ...
                  <div className="flex items-center gap-2 px-3 pb-2 pt-1.5 border-t border-white/20 flex-wrap">
                    {showImageUpload && <MediaPill icon={<Camera className="h-3 w-3" />} label="รูปภาพ" accept="image/*" file={imageFile} onFile={setImageFile} onClear={() => setImageFile(null)} maxMB={10} />}
                    {showAudioUpload && <MediaPill icon={<Mic className="h-3 w-3" />} label="เสียง" accept="audio/*" file={audioFile} onFile={setAudioFile} onClear={() => setAudioFile(null)} maxMB={20} />}
                    ...
                    {showAudioUpload && !audioFile && (
                      isRecording ? (
                        ...
                        <button type="button" onClick={startRecording}
                          ...
                          <Mic className="h-3 w-3" /> อัดเสียง
                        </button>
                      )
                    )}
                  </div>
                  <div className="px-3 pb-3">
                    <button onClick={handleSubmit} disabled={!hasInput || loading}
                      ...
                      {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> กำลังวิเคราะห์...</> : <><Wrench className="h-4 w-4" /> วินิจฉัย</>}
                    </button>
                  </div>
                </div>
                ...
                {vehicleSummary && (
                  <p className="text-xs text-white/25 mt-2 flex items-center gap-1.5">
                    <Check className="h-3 w-3 text-emerald-500" /> บันทึกข้อมูลรถไว้ในอุปกรณ์นี้แล้ว
                  </p>
                )}
              </div>
            )}
              ...
              <div className="bg-[#0D1117]/80 border border-white/10 rounded-2xl overflow-hidden flex flex-col backdrop-blur-sm"
                ...
                <div className="border-t border-white/10 bg-[#080C10]/80 px-3 py-2.5">
                  <div className="flex gap-2 items-center">
                    <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKey}
                      placeholder="ถามคำถามเพิ่มเติม... (กด Enter เพื่อส่ง)" rows={1}
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <button onClick={startNewChat} className="flex items-center gap-1 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                      <Plus className="h-3 w-3" /> แชทใหม่
                    </button>
                  </div>
                </div>
              </div>
            ...
            {!hasMessages && (
              <div className="hidden lg:flex items-center justify-end">
                <button onClick={startNewChat} className="flex items-center gap-1.5 text-xs text-white/25 hover:text-emerald-400 transition-colors">
                  <Plus className="h-3 w-3" /> เริ่มการสนทนาใหม่
                </button>
              </div>
            )}
