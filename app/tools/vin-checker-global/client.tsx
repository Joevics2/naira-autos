...
export default function VINCheckerClient() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('Please enter a VIN number.'); return; }
    if (!isValidVIN(cleaned)) { setError('VIN must be exactly 17 characters — letters (no I, O, Q) and numbers only.'); return; }
    ...
    } catch { setError('Could not reach the VIN database. Check your connection and try again.'); }
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                placeholder="Enter VIN / Chassis Number (17 characters)"
        ...
        {result ? (
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">This VIN is outside the NHTSA database (European/UK origin). Decoded model year from VIN structure{result.modelYear ? `: ${result.modelYear}` : ''}. For full specs, contact the manufacturer.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">VIN decoded — US/North American vehicle confirmed</p>
            ...
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">Verify VIN in all three physical locations</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Where to Find the VIN</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">VIN Structure</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'Country & manufacturer' },
                  { pos: '4–8', name: 'VDS', desc: 'Model, engine, body' },
                  { pos: '9', name: 'Check digit', desc: 'Validates the full VIN' },
