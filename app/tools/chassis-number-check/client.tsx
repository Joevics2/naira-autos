...
const VIN_LOCATIONS = [
  { n: '1', spot: 'Dashboard plate', detail: 'Through the windshield, driver\'s side base.' },
  { n: '2', spot: 'Door jamb sticker', detail: 'Inside driver\'s door frame — must match dashboard.' },
  { n: '3', spot: 'Chassis stamp', detail: 'Stamped on chassis rail under the bonnet.' },
  { n: '4', spot: 'Boot / under seat', detail: 'Some models stamp under boot carpet or front seat.' },
];
...
export default function ChassisNumberCheckerClient() {
  ...
  const decode = async () => {
    ...
    if (!cleaned) { setError('Please enter a chassis number.'); return; }
    if (!isValidVIN(cleaned)) { setError('Chassis number must be exactly 17 characters — letters (no I, O, Q) and numbers only.'); return; }
    ...
    } catch { setError('Could not reach the chassis number database. Check your connection and try again.'); }
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="max-w-2xl mb-8">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input ref={inputRef} type="text" value={vin}
                ...
                className="w-full h-12 px-4 text-sm font-mono tracking-wider border border-border rounded-xl bg-background text-foreground placeholder:text-muted-foreground/60 placeholder:font-sans placeholder:tracking-normal focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all"
              />
              {vin && (
            ...
            <button onClick={decode} disabled={loading || vin.length < 17}
              className={`flex items-center gap-2 px-5 rounded-xl text-sm font-bold transition-all flex-shrink-0 ${!loading && vin.length === 17 ? 'bg-blue-500 hover:bg-blue-400 text-white shadow-lg shadow-blue-500/25' : 'bg-muted text-muted-foreground cursor-not-allowed'}`}>
          ...
          <div className="flex justify-between items-center mt-1">
            <span className="text-xs text-muted-foreground">{vin.length}/17 characters</span>
            {vin.length === 17 && <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">Ready to decode</span>}
          </div>
          {error && (
        ...
        {result ? (
          <div className="space-y-4 max-w-4xl">
            ...
            {vinSource === 'fallback' ? (
              <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">Limited data — non-US vehicle</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/70 mt-1">This chassis number is outside the NHTSA database (European/UK origin). Decoded model year from its structure{result.modelYear ? `: ${result.modelYear}` : ''}. For full specs, contact the manufacturer.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2.5 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">Chassis number decoded — US/North American vehicle confirmed</p>
                <button onClick={reset} className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> New check</button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(result.engineCylinders || result.engineDisplacementL || result.engineHP) && (
                <SpecCard icon={<Zap className="h-3.5 w-3.5 text-orange-500" />} title="Engine">
                  {result.engineCylinders && <Row label="Cylinders" value={result.engineCylinders} />}
                  {result.engineDisplacementL && <Row label="Displacement" value={result.engineDisplacementL + 'L'} />}
                  {result.engineHP && <Row label="HP" value={result.engineHP} />}
                </SpecCard>
              )}
              {(result.manufacturer || result.plantCountry) && (
                <SpecCard icon={<Globe className="h-3.5 w-3.5 text-blue-500" />} title="Origin">
                  {result.manufacturer && <Row label="Maker" value={result.manufacturer} />}
                  {result.plantCountry && <Row label="Country" value={`${getOriginFlag(result.plantCountry)} ${result.plantCountry}`} />}
                  {result.plantCity && <Row label="Plant" value={result.plantCity} />}
                </SpecCard>
              )}
              {(result.modelYear || result.doors || result.transmissionStyle) && (
                <SpecCard icon={<Car className="h-3.5 w-3.5 text-emerald-500" />} title="Vehicle">
                  {result.modelYear && <Row label="Year" value={result.modelYear} />}
                  {result.doors && <Row label="Doors" value={result.doors} />}
                  {result.transmissionStyle && <Row label="Gearbox" value={result.transmissionStyle} />}
                </SpecCard>
              )}
              {(result.abs || result.airBagLocFront) && (
                <SpecCard icon={<Shield className="h-3.5 w-3.5 text-emerald-500" />} title="Safety">
                  {result.abs && <Row label="ABS" value={result.abs} />}
                  {result.airBagLocFront && <Row label="Airbags" value={result.airBagLocFront} />}
                  {result.seatBeltsAll && <Row label="Belts" value={result.seatBeltsAll} />}
                </SpecCard>
              )}
            </div>
            ...
            <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/5 p-4">
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-300 mb-2">Verify the chassis number in all physical locations</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href="/tools/ai-mechanic" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group">
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">Diagnose this car</p>
              ...
              <Link href="/tools/import-duty-calculator" className="flex items-center justify-between gap-2 px-3 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group">
                <p className="text-xs font-bold text-blue-700 dark:text-blue-400">Calculate import duty</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Where to Find the Chassis Number</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wide mb-3">Chassis Number Structure</h3>
              <div className="space-y-2">
                {[
                  { pos: '1–3', name: 'WMI', desc: 'Country & manufacturer' },
                  { pos: '4–8', name: 'VDS', desc: 'Model, engine, body' },
                  { pos: '9', name: 'Check digit', desc: 'Validates the full chassis number' },
                  { pos: '10', name: 'Model year', desc: 'K=2019 · L=2020 · N=2022' },
                  { pos: '11', name: 'Plant', desc: 'Assembly factory code' },
                  { pos: '12–17', name: 'Serial', desc: 'Unique production number' },
                ].map(({ pos, name, desc }) => (
