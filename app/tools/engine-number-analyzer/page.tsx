  ...
  '@graph': [
    ...
    },
    {
      ...
      mainEntity: [
        ...
        { '@type': 'Question', name: 'Do customs authorities check engine numbers when importing a car?', acceptedAnswer: { '@type': 'Answer', text: 'In many countries, yes — customs and border agencies record the engine number alongside the VIN/chassis number during import clearance to confirm the vehicle matches its declared specification. Exact procedures vary by country, so check with your local customs authority before shipping or clearing a vehicle.' } },
        { '@type': 'Question', name: 'Can vehicle registration be rejected over an engine number mismatch?', acceptedAnswer: { '@type': 'Answer', text: 'It can, in most jurisdictions. Vehicle registration and licensing authorities typically check that the engine number on your documents matches the number stamped on the vehicle. An unexplained mismatch, without paperwork for a genuine engine swap, is a common reason registration gets delayed or rejected worldwide.' } },
        { '@type': 'Question', name: 'Can I use this to confirm an engine before importing a used car?', acceptedAnswer: { '@type': 'Answer', text: 'It is a useful first check, confirming the family code matches what the seller claims before you pay. It does not replace a physical pre-purchase inspection or your country\'s official customs/registration process, but it can flag an obvious mismatch early.' } },
      ],
    },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3 text-sm text-muted-foreground leading-relaxed">
                  <p>Yes, and this trips people up constantly. The <strong className="text-foreground">chassis number (VIN)</strong> identifies the whole vehicle as it left the factory, and its format is standardized worldwide under <a href="https://www.iso.org/standard/52200.html" target="_blank" rel="noopener noreferrer nofollow" className="underline underline-offset-2 hover:text-foreground">ISO 3779</a>. The <strong className="text-foreground">engine number</strong> identifies only the engine block, stamped separately, and — unlike the VIN — has no equivalent international standard; format and placement are set by each manufacturer. A car that has had a genuine engine replacement — after a major failure, for example — can legitimately show an engine number that no longer matches what the original paperwork recorded, even though nothing improper happened.</p>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  { q: 'Do customs authorities check engine numbers when importing a car?', a: 'In many countries, yes — customs and border agencies record the engine number alongside the VIN/chassis number during import clearance to confirm the vehicle matches its declared specification. Exact procedures vary by country, so check with your local customs authority before shipping or clearing a vehicle.' },
                  { q: 'Can vehicle registration be rejected over an engine number mismatch?', a: 'It can, in most jurisdictions. Vehicle registration and licensing authorities typically check that the engine number on your documents matches the number stamped on the vehicle. An unexplained mismatch — without paperwork for a genuine engine swap — is a common reason registration gets delayed or rejected worldwide.' },
                  { q: 'Can I use this to confirm an engine before importing a used car?', a: 'It\'s a useful first check — confirming the family code matches what the seller claims before you pay. It doesn\'t replace a physical pre-purchase inspection or your country\'s official customs/registration process, but it can flag an obvious mismatch early.' },
                ].map(({ q, a }) => (
